...
<div class="card shadow">
    <div class="card-body">
        <div class="container-fluid">
            ...
            <form action="{{ route('user.dashboard.akun.action') }}" method="post" id="form-profile" enctype="multipart/form-data" class="{{ isset($user->biodata) && $user->biodata->verified !== 'unverified' ? 'input-none' : '' }}">
                @csrf
                <div class="row">
                    <div class="form-group col-12 iptFile-input">
                        ...
                        <div class="input-group flex-nowrap">
                            ...
                            @if (isset(auth()->user()->biodata) && isset(auth()->user()->avatar) && file_exists(public_path(auth()->user()->avatar)))
                            <input type="hidden" value="{{ asset(auth()->user()->avatar) }}"
                                id="avatar_existing">
                            @endif
                            <button class="input-group-text d-none border-secondary" type="button" data-id-target="avatar">
                                <i class=" fa-regular fa-eye"></i>
                            </button>
                        </div>
                        <span class="keterangan" style="font-size: 12px;">Foto Bebas (PNG, JPEG JPG), Max 500kb</span>
                    </div>
                </div>
                <!-- Submit Button -->
                <div class="row mb-2">
